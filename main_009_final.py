...
cl_columns = ['ORR', 'RECIST', '检查编号', '筛选号', '试验系列名称', '性别', '就诊年龄', '是否吸烟', 'Drug', '病理诊断_文本',
              ...
              '是否死亡', '死亡时间', '死亡原因', '临床试验分期M', '最长径（淋巴结为最短经）', '是否吸烟2', '每日吸烟数量（支）', '吸烟时长（年）', '驱动基因突变',
...
cl_features = cl_features[['Group', 'ORR', 'imageName', '年龄', '性别',	'set',	'是否吸烟', '是否吸烟2','每日吸烟数量' ,
...
selected_ml_features.to_csv(os.path.join(folder, 'selected_ml_features.csv'), index=False)

# ================================== combine ml feature with cl features ==============================
all_features = pd.merge(cl_features, selected_ml_features, on='imageName', how='inner')
all_features = all_features.drop(columns=['是否吸烟2'])
...
ml_feat_start = all_features.columns.get_loc('wavelet2-LLL_gldm_LargeDependenceHighGrayLevelEmphasis')
all_features = all_features.drop(all_features.columns[ml_feat_start+30:], axis=1)
...
grid_search = GridSearchCV(estimator=cls, param_grid=param_grid, cv=5, scoring='roc_auc', verbose=1, n_jobs=-1) # roc_auc
